...
Yes! This guide will install a few useful homebrew applications, including [Universal-Updater cia format](docs/_files/Universal-Updater.cia) [Universal-Updater 3dsx format](docs/_files/Universal-Updater.3dsx), which acts as a homebrew app store.

:::

::: details Can I use this to play games from other regions?

Yes; Luma3DS will automatically ignore the region check for cartridges and installed titles. Some games may need to make use of Luma's [locale emulation feature](_files/3DS_Setting_game_locales.md) in order to work properly on out-of-region consoles.
...
Yes, system transfers can be performed through the official System Transfer function to other consoles with CFW (inconsistencies may occur if the target console is unmodified). Tickets for illegitimate titles (homebrew) will not transfer, but the titles can be made to reappear with [faketik](docs/_files/faketik.3dsx). Make sure that you do not perform a wireless transfer, as this will delete illegitimate titles. CFW will remain on both consoles.
...
Your old setup (including your EmuNAND, if you have one) can usually be directly migrated to boot9strap with no data loss. If you have data that is particularly important to you, it would be a good idea to make a decrypted backup of your save data before upgrading your setup with a tool like [JKSM 3dsx format](docs/_files/JKSM.3dsx), [JKSM cia format](docs/_files/JKSM.cia), and  [JKSM zip includes xml file settings](docs/_files/JKSM_hax.zip). "Note: Rewrite is currently work-in-progress. There may be issues, bugs, and crashes. You're using this at your own risk until I consider it finished. This is a work-in-progress rewrite and partial backport of the Switch version. You have been warned for the time being." qouted from repo

:::

::: details How do I move saves from an existing Gateway setup to a more modern setup?

See [this pdf](_files/Importing_Gateway_saves_to_cia_games_GBAtemp.net-The_Independent_Video_Game_Community.pdf).

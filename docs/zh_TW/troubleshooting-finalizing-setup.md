...
2. 將網路設定中的 DNS 位址設定設為『自動 (Auto)』
3. 將主機靠近您的 WiFi 路由器
4. 開機時按住「L」鍵 +「R」鍵 +「上」鍵 +「A」鍵以開機至安全模式中，並遵循所示指示進行更新
5. 將主機中的 WiFi 連線設定清除，並再重新設定一次
6. 重新啟動您的 WiFi 路由器
...
6. Press and hold (Start), and while holding (Start), power on your console. 這將會啟動 GodMode9
